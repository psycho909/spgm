...
function loadI18n() {
	...
	$.i18n.properties({
		...
		cache: false,
		callback: function() {
			APPLICATION.documentTitle = $.i18n.prop('device.smart_power_meter');
...
function initial() {
	...
	deferreds.push(
		...
		createCodeRadio('#power_phase_container', 'powerPhase', URLS.CODE.LIST.POWER_PHASE, 'no'),
			...
			loadData: function(action, activeRow) {
				if (action == CONSTANT.ACTION.ADD) {
					editForm.formData({
						...
						deviceCategoryId: APPLICATION.codeHelper.deviceCategorySmartPowerMeter.id, 
...
function loadTable(data, callback, settings) {
	...
	data.parameters.deviceCategoryId = APPLICATION.codeHelper.deviceCategorySmartPowerMeter.id;
