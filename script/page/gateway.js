...
function loadI18n() {
	...
	$.i18n.properties({
		...
		cache: false,
		callback: function() {
			APPLICATION.documentTitle = $.i18n.prop('device.gateway');
			...
			$('#server_ip_label').text($.i18n.prop('device.ip.server'));
			...
			loadData: function(action, activeRow) {
				if (action == CONSTANT.ACTION.ADD) {
					editForm.formData({
						...
						deviceCategoryId: APPLICATION.codeHelper.deviceCategoryGateway.id, 
...
function loadTable(data, callback, settings) {
	...
	data.parameters.deviceCategoryId = APPLICATION.codeHelper.deviceCategoryGateway.id;
